...
quanti riferimenti N<sub>T</sub>=N<sub>W</sub>+N<sub>R</sub> (N<sub>R</sub> per la lettura e N<sub>W</sub> per la scrittura dei dati) produce il programma proposto (non considerare i fetch delle istruzioni)?
...
I puntatori/indici hanno dimensione di 32 bit e i blocchi disco hanno dimensione di 4KB. 
...
B) Dato un file binario di dimensione 20490.5KB, calcola esattamente quanti blocchi indice e blocchi dati sta utilizzando il file. 
Calcola anche la frammentazione interna (per i blocchi dati).
C) Considera lo stesso file della Domanda B, dove l'operazione `lseek(fd, offset, SEEK_END)` viene chiamata per posizionare 
l'offset del file (*SEEK_END* significa che l'offset si riferisce alla fine del file) per la successiva operazione di lettura/scrittura. 
